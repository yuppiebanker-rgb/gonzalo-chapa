...
module.exports = async function handler(req, res) {
  ...
  const subject = `Nueva Cotización — ${name}`;
...
<div class="wrap">
  ...
  ${service_type ? `<div class="field"><span class="label">Servicio</span><span class="value">${service_type}</span></div>` : ''}
  ${event_date ? `<div class="field"><span class="label">Fecha del evento</span><span class="value">${event_date}</span></div>` : ''}
  ${budget ? `<div class="field"><span class="label">Presupuesto</span><span class="value">${budget}</span></div>` : ''}
  ...
  if (!process.env.RESEND_API_KEY) {
    console.log('[notify-quote] RESEND_API_KEY not set — logging to console');
    console.log({ subject, to: TO, name, email, phone, service_type, event_date, budget, message });
  ...
  try {
    ...
    if (error) {
      console.error('[notify-quote] Resend error:', error);
      return res.status(500).json({ error: 'Failed to send email' });
    }

    return res.status(200).json({ ok: true, id: data?.id });
  } catch (err) {
    console.error('[notify-quote] Unexpected error:', err);
    return res.status(500).json({ error: 'Failed to send email' });
  }
};
